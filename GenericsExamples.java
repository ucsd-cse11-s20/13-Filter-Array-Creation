...
class StartsWithWelcome implements Checker<TextTweet> {
  public boolean check(TextTweet t) {
    return t.content.indexOf("#welcome") == 0;
  }
}
...
class GenericsExamples {
  ...
  int countInQuadrant1(Point[] pts) {
    return count(pts, new InQuad1());
  }

  TextTweet t1 = new TextTweet("joepolitz", "#welcome to CSE11, incoming students!");
  ...
  int result = this.countNear0(pts);


  Point[] pts2 = {new Point(-1, 10), new Point(1, -1), new Point(2, 3)};
  int result2 = this.countInQuadrant1(pts2);
